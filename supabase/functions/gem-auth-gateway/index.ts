...
const CANONICAL_ORIGIN = "https://www.gemcybersecurityassist.com";
const CORS_HEADERS = {
...
async function signToken(
  ...
  if (!validSessionVersion(sessionVersion)) {
  ...
  const header = b64url(
    JSON.stringify({ alg: "HS256", typ: "JWT", kid: "gem-auth-v2" }),
  );
...
async function audit(
  userId: string | null,
  action: "login" | "failed_login" | "password_change",
...
async function userContext(userId: string, expectedSessionVersion?: number) {
  const { data: user, error } = await db
    .from("users")
    .select(
      "id,email,role,status,organizationId,isActive,isEmailVerified,sessionVersion",
    )
    .eq("id", userId)
    .maybeSingle();
  ...
  if (entitlements.error) {
    throw new GatewayError(503, "DATABASE_ERROR", entitlements.error.message);
  }
  if (portfolio.error) {
    throw new GatewayError(503, "DATABASE_ERROR", portfolio.error.message);
  }
...
async function login(emailInput: unknown, passwordInput: unknown) {
  ...
  if (!user || !valid) {
    await audit(user?.id ?? null, "failed_login", {
      email,
...
async function passwordRecoveryRequest(emailInput: unknown) {
  const email =
    typeof emailInput === "string" ? emailInput.trim().toLowerCase() : "";
  if (!email || email.length > 254 || !email.includes("@")) {
    return { accepted: true, recovery: "recorded" };
  }
  const { data: user, error } = await db
    .from("users")
    ...
    .maybeSingle();
  if (error) throw new GatewayError(503, "DATABASE_ERROR", error.message);
  const eligible = Boolean(user?.isActive && user.status === "active");
  await audit(user?.id ?? null, "password_change", {
    flow: "forgot_password_request",
    accountEligible: eligible,
    delivery: eligible
      ? "protected_command_center_recovery"
      : "not_applicable",
  });
  return { accepted: true, recovery: "recorded" };
}
...
Deno.serve(async (req) => {
  ...
  if (req.method === "GET") {
    return json({
      ...
      version: "2.0.0",
      sessionVersionEnforced: true,
      legacyUnversionedSessionsAccepted: false,
    });
  }
  if (req.method !== "POST") {
  ...
  try {
    ...
    if (action === "verify") {
      ...
      return json({
        session: await userContext(claims.userId, claims.sessionVersion),
      });
    }
    if (action === "bootstrap_status") return json(await bootstrapStatus());
    if (action === "password_recovery_request") {
      return json(await passwordRecoveryRequest(body.email));
    }
