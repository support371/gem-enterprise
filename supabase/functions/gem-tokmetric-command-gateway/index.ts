...
async function snapshot(actor: { id: string; role: string }) {
  ...
  const [
    ...
    db.from("tokmetric_connectors")
      .select("id,provider,state,displayName,externalAccountId,grantedScopes,lastHealthAt,updatedAt")
    ...
    db.from("tokmetric_approval_requests")
      .select("id,contentId,contentVersionId,requestedById,requiredRole,action,objectHash,state,expiresAt,createdAt,updatedAt")
      .eq("workspaceId", WORKSPACE_ID)
      .order("updatedAt", { ascending: false })
      .limit(50),
    db.from("tokmetric_publish_jobs")
      .select("id,connectorId,contentId,contentVersionId,internalState,externalState,scheduledFor,createdAt,updatedAt")
      .eq("workspaceId", WORKSPACE_ID)
      .order("updatedAt", { ascending: false })
      .limit(50),
    ...
    db.from("tokmetric_audit_events")
      .select("id,action,entityType,entityId,outcome,sourceChannel,safeMetadata,createdAt")
      .eq("workspaceId", WORKSPACE_ID)
      .order("createdAt", { ascending: false })
      .limit(50),
  ...
  if (failed?.error) throw new GatewayError(503, "DATABASE_ERROR", failed.error.message);
  if (credentialsResult.error) {
    throw new GatewayError(503, "DATABASE_ERROR", credentialsResult.error.message);
  }
  ...
  const postingScopes = new Set(postingConnector?.grantedScopes ?? []);
  ...
  const approvedContentCount = contents.filter(
    (content) => content.state === "APPROVED",
  ).length;
  const pendingApprovalCount = approvals.filter(
    (approval) => approval.state === "APPROVAL_REQUIRED",
  ).length;
  ...
  if (!postingConnector) blockers.push("CONTENT_POSTING_CONNECTOR_NOT_CONNECTED");
...
async function createDraft(
  ...
  if (contentInsert.error) {
    throw new GatewayError(503, "CONTENT_CREATE_FAILED", contentInsert.error.message);
  }
  ...
  await Promise.all([
    ...
    domainEvent("content", contentId, "CONTENT_DRAFT_CREATED", {
      versionId,
      objectHash: hash,
    }),
  ]);
...
async function createVersion(
  ...
  if (insert.error) {
    throw new GatewayError(503, "CONTENT_VERSION_CREATE_FAILED", insert.error.message);
  }
  ...
  if (update.error) {
    await db.from("tokmetric_content_versions").delete().eq("id", versionId);
    throw new GatewayError(503, "CONTENT_UPDATE_FAILED", update.error.message);
  }
  ...
  await domainEvent("content", contentId, "CONTENT_VERSION_CREATED", {
    versionId,
    objectHash: hash,
    version,
  });
...
async function runReview(
  ...
  if (!content?.currentVersionId) {
    throw new GatewayError(404, "CONTENT_NOT_FOUND", "Content or active version not found");
  }
  ...
  if (!version) {
    throw new GatewayError(409, "CONTENT_VERSION_MISSING", "Content version not found");
  }
  ...
  if (insert.error) {
    throw new GatewayError(503, "COMPLIANCE_REVIEW_FAILED", insert.error.message);
  }
  const update = await db.from("tokmetric_contents")
...
async function requestApproval(
  ...
  if (expiresAt && !Number.isFinite(expiresAt.getTime())) {
    throw new GatewayError(400, "INVALID_EXPIRY", "Approval expiry is invalid");
  }
  ...
  if (insert.error) {
    throw new GatewayError(503, "APPROVAL_CREATE_FAILED", insert.error.message);
  }
  const update = await db.from("tokmetric_contents")
...
function roleCanDecide(actorRole: string, requiredRole: string) {
  ...
  return ADMIN_ROLES.includes(actorRole as (typeof ADMIN_ROLES)[number]);
}
...
async function decideApproval(
  ...
  if (!["approve", "reject", "revoke"].includes(decision)) {
    throw new GatewayError(400, "INVALID_DECISION", "Approval decision is invalid");
  }
  const reason = requiredText(body.reason, "reason", 10, 2_000);

  const { data: approval, error: approvalError } = await db
    .from("tokmetric_approval_requests")
    .select("id,contentId,contentVersionId,requestedById,requiredRole,objectHash,state,expiresAt")
    .eq("workspaceId", WORKSPACE_ID)
    .eq("id", approvalId)
    .maybeSingle();
  ...
  if (!approval?.contentId || !approval.contentVersionId) {
    throw new GatewayError(404, "APPROVAL_NOT_FOUND", "Approval request not found");
  }
  if (approval.state !== "APPROVAL_REQUIRED") {
    throw new GatewayError(409, "APPROVAL_STATE_INVALID", "Approval is not awaiting a decision");
  }
  ...
  if (!roleCanDecide(actor.role, approval.requiredRole)) {
    throw new GatewayError(403, "APPROVER_ROLE_REQUIRED", "Required approver role is missing");
  }
  ...
  if (approvalUpdate.error) {
    throw new GatewayError(503, "APPROVAL_UPDATE_FAILED", approvalUpdate.error.message);
  }
  const contentUpdate = await db.from("tokmetric_contents")
    .update({ state: nextState, updatedAt: now })
    .eq("id", approval.contentId)
  ...
  if (contentUpdate.error) {
    throw new GatewayError(503, "CONTENT_UPDATE_FAILED", contentUpdate.error.message);
  }
  ...
  await Promise.all([
    ...
    domainEvent("approval", approvalId, `APPROVAL_${decision.toUpperCase()}`, {
      contentId: approval.contentId,
      contentVersionId: approval.contentVersionId,
    }),
  ]);
...
async function publishPreflight(
  ...
  const scopes = new Set(connector?.grantedScopes ?? []);
  const blockers: string[] = [];
  if (workspace.globalEmergencyLock) blockers.push("GLOBAL_EMERGENCY_LOCK");
  if (workspace.connectorDisabled) blockers.push("CONNECTORS_DISABLED");
  if (workspace.publishingDisabled) blockers.push("WORKSPACE_PUBLISHING_DISABLED");
...
async function dispatch(
  ...
  if (typeof body.operation !== "string") {
    throw new GatewayError(
      400,
      "INVALID_OPERATION",
      "TokMetric command operation is required",
    );
  }
  ...
  throw new GatewayError(400, "INVALID_OPERATION", "Unknown TokMetric operation");
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.error(
      "tokmetric_command_gateway_internal_error",
      error instanceof Error ? error.name : "unknown",
    );
    return json(
      {
        error: "TokMetric command gateway unavailable",
        code: "INTERNAL_ERROR",
      },
      500,
    );
