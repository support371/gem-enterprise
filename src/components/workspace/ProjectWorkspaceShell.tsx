...
export function ProjectWorkspaceShell({ environment, project, workspace, membership, permissions }: ProjectWorkspaceShellProps) {
  ...
  const workspaceHref = `/app/workspace?workspace=${encodeURIComponent(workspace.id)}`;
  ...
  const actions = current.id === "overview"
    ? available.filter((item) => item.id !== "overview").map((item) => ({ label: item.label, description: item.description, href: `${base}/${item.id}` }))
    : current.destinations.map((item) => ({ ...item, href: scoped(item.href) }));
  const navItems = available.map((item) => ({
    id: item.id,
    label: item.label,
  ...
  return (
    <div className="pb-12">
      <a
        ...
        className="sr-only z-[100] rounded-lg bg-cyan-300 px-4 py-2 font-semibold text-slate-950 focus:not-sr-only focus:fixed focus:left-4 focus:top-4"
      >
        Skip to project content
      </a>

      <div className="xl:grid xl:grid-cols-[300px_minmax(0,1fr)] xl:gap-5">
        ...
        <main id="workspace-main-content" tabIndex={-1} className="min-w-0 space-y-5 outline-none">
          ...
          <header className="rounded-2xl border border-white/10 bg-slate-950/70 p-4 sm:p-5">
            <nav aria-label="Breadcrumb" className="mb-4 flex flex-wrap items-center gap-1.5 text-xs text-slate-500">
              ...
              <Link href={workspaceHref} className="max-w-48 truncate rounded px-1 py-0.5 hover:text-cyan-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
                {workspace.name}
              </Link>
              <ChevronRight aria-hidden="true" className="h-3.5 w-3.5" />
              <Link href={base} className="max-w-48 truncate rounded px-1 py-0.5 hover:text-cyan-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
                {project.name}
              </Link>
              {current.id !== "overview" && (
                <>
                  <ChevronRight aria-hidden="true" className="h-3.5 w-3.5" />
                  <span aria-current="page" className="font-semibold text-slate-300">{current.label}</span>
                </>
              )}
            </nav>
            ...
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              <div className="min-w-0">
                <p className="truncate text-xs font-semibold uppercase tracking-[.16em] text-cyan-300">
                  {workspace.organization.name} · {workspace.name}
                </p>
                <h1 className="mt-1 truncate text-xl font-bold text-white sm:text-2xl">{project.name}</h1>
                <p className="mt-1 line-clamp-2 max-w-3xl text-sm text-slate-400">{project.summary}</p>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                <Badge className="border-emerald-400/25 bg-emerald-400/10 text-emerald-300">
                  <span className="mr-2 h-1.5 w-1.5 rounded-full bg-emerald-300" />Live workspace
                </Badge>
                <Badge variant="outline" className="border-white/15 text-slate-300">{membership.role?.name ?? "Member"}</Badge>
                <Link href={base} className="inline-flex items-center gap-2 rounded-lg border border-white/10 px-3 py-1.5 text-xs font-medium text-slate-300 hover:border-cyan-400/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300">
                  <RefreshCw className="h-3.5 w-3.5" />Project home
                </Link>
              </div>
            </div>
          </header>

          <section aria-labelledby="environment-heading" className="overflow-hidden rounded-2xl border border-cyan-400/20 bg-[radial-gradient(circle_at_top_right,rgba(34,211,238,.13),transparent_38%),linear-gradient(145deg,rgba(15,23,42,.98),rgba(2,6,23,.98))] p-5 sm:p-7">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[.18em] text-cyan-300">{current.audience}</p>
                <h2 id="environment-heading" className="mt-2 text-2xl font-bold text-white sm:text-3xl">{current.label} dashboard</h2>
                <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-300">{current.description}</p>
              </div>
              <Badge variant="outline" className="w-fit border-white/15 text-slate-300"><ShieldCheck className="mr-1.5 h-3.5 w-3.5" />Project scoped</Badge>
            </div>
          </section>

          <section aria-label="Project metrics" className="grid grid-cols-2 gap-3 xl:grid-cols-4">
            {metrics.map(({ label, value, helper, Icon }) => (
              <article key={label} className="rounded-2xl border border-white/10 bg-card p-4 sm:p-5">
                <div className="flex items-start justify-between"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">{label}</p><span className="flex h-8 w-8 items-center justify-center rounded-lg bg-cyan-400/10"><Icon className="h-4 w-4 text-cyan-300" /></span></div>
                <p className="mt-4 text-2xl font-bold text-white sm:text-3xl">{value}</p><p className="mt-1 text-xs text-slate-500">{helper}</p>
              </article>
            ))}
          </section>
