...
export function StorefrontPage({ storefront }: StorefrontPageProps) {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <StorefrontNavigation active={storefront} />
      ...
      <section className="relative overflow-hidden border-b border-white/10 bg-[#07101c] py-20 md:py-28">
        ...
        <div className="container relative z-10 mx-auto grid max-w-7xl gap-12 px-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
            ...
            <div className="mt-9 flex flex-col gap-4 sm:flex-row">
              ...
              {definition.externalUrl && definition.externalLabel && (
                <Button asChild size="lg" variant="outline" className="rounded-full border-white/20 px-8 text-white hover:bg-white/10">
                  <a href={definition.externalUrl} target="_blank" rel="noreferrer">
                    {definition.externalLabel} <ExternalLink className="ml-2 h-5 w-5" />
                  </a>
                </Button>
              )}
            </div>
          ...
          <aside className="rounded-3xl border border-cyan-500/20 bg-white/[0.035] p-7 shadow-2xl shadow-cyan-950/20">
            <div className="flex items-start justify-between gap-4">
              <div>
                <div className="text-xs uppercase tracking-[0.2em] text-slate-500">Store status</div>
                <div className="mt-2 text-2xl font-black text-white">{definition.status}</div>
              </div>
            ...
            <div className="mt-6 grid grid-cols-2 gap-4">
              ...
              <div className="rounded-2xl border border-white/10 bg-black/20 p-4">
                ...
                <div className="mt-2 text-3xl font-black text-white">{Math.max(definition.categories.length - 1, 0)}</div>
              </div>
            </div>
            <div className="mt-6 grid gap-3 text-sm text-slate-300">
              <div className="flex items-start gap-3">
                <ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-cyan-400" />
                <span>The GEM website remains the canonical product and support destination.</span>
              </div>
              <div className="flex items-start gap-3">
                <PackageCheck className="mt-0.5 h-5 w-5 shrink-0 text-cyan-400" />
                <span>External checkout is shown only when an approved destination is available.</span>
              </div>
            </div>
          </aside>
        </div>
      ...
      <section id="products" className="container mx-auto max-w-7xl px-6 py-20">
        <div className="mb-10">
          ...
          <p className="mt-4 max-w-3xl text-lg leading-relaxed text-slate-400">
            Search, filter, review pricing, open an approved checkout, or add products to a GEM order request.
          </p>
        </div>
