...
import { ConvertedClientOnboarding } from "@/components/market/ConvertedClientOnboarding";
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-white/10 bg-white/[0.03] p-6">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          ...
          <div className="rounded-xl border border-white/10 bg-black/15 p-4"><p className="text-xs text-slate-500">Commercial gate</p><p className="mt-1 text-sm font-semibold text-white">{converted ? "Payment verified — onboarding review" : submission.status === "APPROVED" ? "Eligible when merchant is ready" : "Requires APPROVED status"}</p></div>
        </div>
      </section>
      ...
      {converted && <ConvertedClientOnboarding submission={submission} />}
    </div>
  );
}
