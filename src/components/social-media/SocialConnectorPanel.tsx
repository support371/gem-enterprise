...
export function SocialConnectorPanel({ providers }: { providers: SafeSocialOAuthReadiness[] }) {
  ...
  const connectorByProvider = new Map(connectors.map((connector) => [connector.provider, connector]));
    ...
    <section className="rounded-2xl border border-cyan-300/20 bg-cyan-300/[0.04] p-5">
      ...
      <div className="mt-5 grid gap-3 lg:grid-cols-2">
        {providers.map((provider) => {
          const connector = connectorByProvider.get(provider.provider);
          return (
            <article key={provider.provider} className="rounded-xl border border-white/10 bg-white/[0.03] p-4">
              <div className="flex flex-wrap items-start justify-between gap-3">
                ...
                <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs font-semibold uppercase tracking-[0.1em] text-white/70">
                  {connector ? label(connector.state) : provider.readyToAuthorize ? "Authorization required" : "Blocked"}
                </span>
              </div>
              ...
              {connector ? (
                <div className="mt-4 space-y-3">
                  <div className="rounded-lg border border-emerald-300/15 bg-emerald-300/[0.04] p-3 text-sm text-white/60">
                    <p>Account: {connector.externalAccountId || "profile discovery pending"}</p>
                    <p className="mt-1">Granted scopes: {connector.grantedScopes.join(", ") || "not returned by provider"}</p>
                    <p className="mt-1">Last health record: {connector.lastHealthAt ? new Date(connector.lastHealthAt).toLocaleString() : "not recorded"}</p>
                  </div>
                  <button
                    type="button"
                ...
                >
                  Authorize account <ExternalLink className="h-3.5 w-3.5" />
                </button>
              )}
            </article>
          );
