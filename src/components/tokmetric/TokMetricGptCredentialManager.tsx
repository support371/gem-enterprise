...
export function TokMetricGptCredentialManager() {
    ...
    try {
      const response = await fetch("/api/tokmetric/gpt-credentials", {
        ...
        body: JSON.stringify({
          ...
          reason: reason.trim(),
          expiresInDays,
          confirmWorkspaceId: WORKSPACE_ID,
        }),
      });
  ...
  async function revokeCredential(credential: Credential) {
    const reasonInput = window.prompt(
      `Enter a revocation reason for “${credential.label}” (minimum 10 characters):`,
    );
    if (!reasonInput || reasonInput.trim().length < 10) return;

    const confirmation = window.prompt(
      `Type the exact credential label to confirm revocation:\n${credential.label}`,
    );
    if (confirmation !== credential.label) {
                ...
                >
                  ...
                  {credential.status === "active" ? (
                    <button
                      ...
                      className="mt-3 inline-flex items-center gap-2 text-xs font-semibold text-rose-300 hover:text-rose-200"
                    >
