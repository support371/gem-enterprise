...
export const ADMIN_ACCESS_SESSION_KEY = "gem_admin_access_authorization_v2";
...
export function buildAdminAccessSql({
  ...
  const email = escapeSqlLiteral(ADMIN_ACCESS_EMAIL);
  const hash = escapeSqlLiteral(tokenHash);
  const request = escapeSqlLiteral(requestId);
  const expiryIso = escapeSqlLiteral(expiry.toISOString());
