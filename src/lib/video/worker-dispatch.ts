...
import {
  claimVerifiedVideoRendersForFinalization,
  createVideoRenderJob,
  releaseVideoRenderFinalizationClaim,
...
export async function finalizeVerifiedWorkerRenders(input: {
  ...
  const claimed = await claimVerifiedVideoRendersForFinalization({
    limit: input.limit ?? 5,
  });
  ...
  for (const record of claimed) {
    ...
    if (!claimId || !record.externalPromptId || !record.requestedById) {
      if (claimId) {
        await releaseVideoRenderFinalizationClaim({
          id: record.id,
          claimId,
