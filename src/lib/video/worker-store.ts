...
export type TrustedWorkerRenderJob = {
  id: string;
  workspaceId: string;
  contentId: string;
...
type WorkerJobRow = TrustedWorkerRenderJob;
...
export async function listTrustedWorkerRenderJobs(limit = 10) {
  const boundedLimit = Math.min(Math.max(limit, 1), 20);
  try {
    return await db.$queryRaw<WorkerJobRow[]>(Prisma.sql`
      SELECT
        jobs.id,
