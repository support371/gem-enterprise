...
export async function cancelTikTokPublish(accessToken: string, publishId: string) {
  const response = await callTikTok(accessToken, "/v2/post/publish/cancel/", z.object({ error: errorSchema }), { publish_id: publishId });
