...
type Story = { item: { slug: string; title: string; summary: string | null; aiSummary: string | null; externalUrl: string; category: string; author: string | null; imageUrl: string | null; imageAlt: string | null; videoUrl: string | null; publishedAt: string; source: { name: string; siteUrl: string | null } | null } };
...
export default async function StoryPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article className="min-h-screen bg-[#020817] px-4 py-10 text-white"><div className="mx-auto max-w-4xl"><Link href="/intel/news" className="text-sm text-cyan-300"><ArrowLeft className="mr-2 inline h-4 w-4" />Back to GEM News</Link>
    ...
    {story.imageUrl && <div className="relative mt-8 overflow-hidden rounded-3xl border border-white/10">
      ...
      {story.videoUrl && <PlayCircle className="absolute left-1/2 top-1/2 h-16 w-16 -translate-x-1/2 -translate-y-1/2 text-white drop-shadow-lg" />}
    </div>}
