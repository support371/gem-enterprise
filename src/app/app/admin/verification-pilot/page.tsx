...
export default function VerificationPilotPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <h1 className="text-2xl font-bold text-white">
              Reviewer designation and pilot readiness
            </h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button asChild variant="outline">
            <Link href="/app/admin/verification-pilot/evidence">
              <ClipboardCheck className="mr-2 h-4 w-4" /> Evidence report
            </Link>
          </Button>
          <Button asChild className="bg-cyan-400 text-black hover:bg-cyan-300">
            <Link href="/review/verification">
              Open review queue <ExternalLink className="ml-2 h-4 w-4" />
            </Link>
          </Button>
        </div>
      </div>
      ...
      {notice && (
        <div className="rounded-xl border border-green-500/25 bg-green-500/10 p-4 text-sm text-green-300">
          {notice}
        </div>
      )}
      ...
      {loading ? (
        <div className="flex items-center gap-2 py-20 text-slate-400">
          <Loader2 className="h-5 w-5 animate-spin" /> Evaluating pilot controls
        </div>
      ) : (
        <>
          <section
                ...
                <div>
                  <h2 className="text-lg font-semibold text-white">
                    {readiness?.ready
                      ...
                      : "Pilot controls need attention"}
                  </h2>
                  <p className="mt-1 text-sm text-slate-400">
                    This check is read-only and does not mutate production data.
                  </p>
                </div>
              </div>
              <Badge
          ...
          <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
            <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  <h2 className="font-semibold text-white">Eligible account selection</h2>
                  <p className="mt-1 text-xs text-slate-500">
                    Only active, email-verified accounts can be promoted to reviewer roles.
                  </p>
                </div>
                        ...
                        <div className="text-right">
                          ...
                          <p
                            className={`mt-1 text-[11px] ${
                              eligible ? "text-green-400" : "text-amber-400"
                            }`}
                          >
                            {eligible ? "Eligible" : "Not eligible for promotion"}
                          </p>
                        </div>
                      </div>
                    </button>
            ...
            <div className="h-fit rounded-2xl border border-white/10 bg-white/[0.03] p-5 xl:sticky xl:top-20">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-semibold text-white">Assign role</h2>
              </div>
              {!selected ? (
                <p className="mt-4 text-sm leading-6 text-slate-400">
                  Select an account to begin. No role changes occur until the email and reason are confirmed.
                </p>
              ) : (
                <div className="mt-5 space-y-4">
                  <div className="rounded-lg border border-white/10 bg-black/10 p-3 text-sm">
                    <p className="font-medium text-white">{displayName(selected)}</p>
                    <p className="text-xs text-slate-500">{selected.email}</p>
                  </div>
                  <div className="space-y-1.5">
                    <Label htmlFor="requestedRole">New role</Label>
                    <select
                      id="requestedRole"
                      value={requestedRole}
                      onChange={(event) =>
                        setRequestedRole(event.target.value as AssignableRole)
                      }
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="confirmEmail">Confirm target email</Label>
                    <Input
                      id="confirmEmail"
                      value={confirmEmail}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea
                      ...
                      placeholder="Explain why this account requires the role."
                    />
                  </div>
                  <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-3 text-xs leading-5 text-amber-100/80">
                    Role changes are audited. The target account must sign out and sign in again for navigation to reflect a promotion. Demotion takes effect immediately for protected APIs.
                  </div>
                  <Button
