...
import { consumeWorkspaceOwnerInvitation } from "@/lib/workspace-owner-invitations-gateway";
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) {
    return json(
      {
        error: "Workspace owner account details are invalid.",
  ...
  try {
    const result = await consumeWorkspaceOwnerInvitation({
      tokenHash,
      passwordHash,
    ...
    return json({ error: "Workspace owner account could not be created." }, 503);
  }
}
