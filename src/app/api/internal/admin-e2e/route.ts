...
import { e2eProvisionGateway } from "@/lib/supabase-gateway";
...
export async function POST(request: NextRequest) {
  ...
  const suffix = createHash("sha256").update(`e2e:${token}`).digest("hex").slice(0, 24);
  ...
  try {
    ...
    checks.testLogin = testLogin.response.status === 200 && testLogin.body.success === true;
    checks.testCookie =
    ...
    const testSessionBody = JSON.parse(testSession.text || "{}") as Record<string, unknown>;
    checks.testSession =
    ...
    const testUsersBody = JSON.parse(testUsers.text || "{}") as { users?: unknown[] };
    checks.testUsersApi =
    ...
    const testStatsBody = JSON.parse(testStats.text || "{}") as Record<string, unknown>;
    checks.testStatsApi =
    ...
    const readinessBody = JSON.parse(testReadiness.text || "{}") as Record<string, unknown>;
    ...
    const realSessionBody = JSON.parse(realSession.text || "{}") as Record<string, unknown>;
    ...
    if (testUserId) {
      try {
        const cleanup = await e2eProvisionGateway<{ ok: boolean; cleaned: boolean }>(
          "cleanup",
          { e2eToken: token, userId: testUserId },
        );
  ...
  const passed = Object.keys(checks).length === 12 && Object.values(checks).every(Boolean);
